...
export async function saveContractsToCloud(arr){const list=Array.isArray(arr)?arr:[];const db=getDB();const col=collection(db,'dfs.contracts');await Promise.all(list.map(c=>{let id=c?.id||(crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${Math.random().toString(36).slice(2)}`);const sanitized={...c,id,clientId:CLIENT_ID,updatedAt:ts()};if(!c?.createdAt) sanitized.createdAt=new Date().toISOString();return setDoc(doc(col,id),sanitized,{merge:true});}));return{ok:true};}
...
export async function saveCustomersToCloud(arr){const list=Array.isArray(arr)?arr:[];const db=getDB();const col=collection(db,'dfs.customers');await Promise.all(list.map(c=>{let id=c?.id||(crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${Math.random().toString(36).slice(2)}`);const sanitized={...c,id,clientId:CLIENT_ID,updatedAt:ts()};if(!c?.createdAt) sanitized.createdAt=new Date().toISOString();return setDoc(doc(col,id),sanitized,{merge:true});}));return{ok:true};}
